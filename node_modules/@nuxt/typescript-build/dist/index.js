    ...
    this.extendBuild((config, { isClient, isModern }) => {
        ...
        config.module.rules.push(...['ts', 'tsx'].map(ext => ({
            ...
            use: [
                ...
                {
                    loader: 'ts-loader',
                    options: Object.assign({ transpileOnly: true, [`append${ext.charAt(0).toUpperCase() + ext.slice(1)}SuffixTo`]: [/\.vue$/] }, (options.loaders && options.loaders[ext]))
                }
            ]
        })));
        if (options.typeCheck && isClient && !isModern) {
            ...
            config.plugins.push(new ForkTsCheckerWebpackPlugin(Object.assign({
                vue: true,
                tsconfig: path_1.default.resolve(this.options.rootDir, 'tsconfig.json'),
                formatter: 'codeframe',
